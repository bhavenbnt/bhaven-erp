...
export default function AdminInquiries() {
  ...
  const filtered = inquiries.filter((inq: any) => {
    ...
    if (search) {
      ...
      if (!(inq.users?.company_name || '').toLowerCase().includes(q)
        && !(inq.users?.name || '').toLowerCase().includes(q)
        && !(inq.product_name || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <Layout
      ...
      action={
        <div style={s.topBar}>
          <div style={s.topLeft}>
            <h1 style={s.pageTitle}>문의 관리</h1>
            <span style={s.totalBadge}>{inquiries.length}건</span>
          </div>
          <div style={s.searchWrap}>
            {Icons.message({ size: 14, color: '#BBB' })}
            <input style={s.searchInput} placeholder="업체명, 제품명 검색"
              ...
              {paginated.map((inq: any) => {
                ...
                return (
                  <tr key={inq.inquiry_id} style={s.tr}>
                    ...
                    <td style={{ ...s.td, textAlign: 'center' }}>
                      <button style={s.detailBtn} onClick={() => router.push(`/admin/inquiries/${inq.inquiry_id}`)}>
...
const s: Record<string, React.CSSProperties> = {
  ...
  searchWrap: {
    ...
    border: '1px solid #EEEEEE', minWidth: 240,
  },
