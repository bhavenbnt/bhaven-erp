...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: user, error: userError } = await supabase
      .from('users')
      .select('user_id, email, name, role, is_active, deleted_at')
      .eq('email', email)
      .single();

    if (userError || !user || !user.is_active || user.deleted_at) {
      return Response.json({ error: '사용할 수 없는 계정입니다.' }, { status: 403 });
    }
